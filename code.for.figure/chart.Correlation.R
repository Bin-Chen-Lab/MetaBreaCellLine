# I steal this code from PerformanceAnalytics to visualize correlation matrix
# Note: the heatmap bar generated by this file only maps value from 0.2 to 1.
# Definitely I need to revise the code to make the function more flexible 
...
chart.Correlation <-
function (R, histogram = TRUE, method=c("pearson", "kendall", "spearman"), cor.range=c(0.2,1),cex.cor.tuning=1.6,...)
{ # @author R Development Core Team
  # @author modified by Peter Carl
    ...
    panel.cor <- function(x, y, digits=2, prefix="", use="pairwise.complete.obs", method="spearman", cex.cor, ...)
    {
        ...
        high <- cor.range[2]
        k    <- 100/(high-low)
        idx <- as.integer(k * r + 1 - low * k)    # mapping (low,high) to (1,101)
        rect(par("usr")[1],par("usr")[3],par("usr")[2],par("usr")[4],col = my_palette[idx])

        
        
        txt <- format(c(r, 0.123456789), digits=digits)[1]
        txt <- paste(prefix, txt, sep="")
        if(missing(cex.cor)) cex <- cex.cor.tuning/strwidth(txt)
        ...
        text(0.5, 0.5, txt, cex = cex * (abs(r) + 0.6) / 1.3)
    ...
    panel.scatter.plot <- function(x, y,...){
    ...
    if(histogram)
        ...
        pairs(x, gap=0, lower.panel=panel.scatter.plot, upper.panel=panel.cor,cex.labels=7,xlim=c(0,0.6),ylim=c(0,0.6),cex.axis=2,font.axis=2	) 
}
