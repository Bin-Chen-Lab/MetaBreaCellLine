...
############################################################ 
#### Fig 4a: right  panel, boxplot of expression correlation with Basal-like MET500 breast cancer samples, MDAMB231 and HCC70
###########################################################
...
ggplot(long.df,aes(x=cell.line.name,y=value,color=factor(color))) + geom_boxplot(lwd=1.2,outlier.shape = NA) + geom_point(size=4,aes(fill=color),position=position_jitterdodge(jitter.width = 0.2))  + ggplot.style + ylab('correlation') + xlab('') + scale_color_manual(values=c('red'='red','black'='black'))
...
############################################################ 
#### Fig 4a: left  panel, ranking cell lines according to transcriptome similarity with Basal-like MET500 breast cancer samples
###########################################################
m              <- apply(expr.cor.matrix,1,median) %>% sort
names(m)       <- gsub(x=names(m),replacement = '',pattern = '_BREAST')
...
ggplot(df) + geom_point(aes(x=rank,y=median.correlation,color=factor(color),size=size),show.legend=F) + ggplot.style + ylab('transcriptome similarity')+ xlab('rank') + scale_color_manual(values=c('red'='red','black'='black')) + scale_size_area(max_size = 4)
ggsave(filename = '/Users/liuke/OneDrive/OneDrive - Michigan State University/Project/Cancer2CellLine/Manuscript/section.Figure/Section3/MET500.basal.like.sample.vs.CCLE.expr.correlation.rank.plot.pdf',width=20,height=15)




#####################################################################
#### Fig 4a: the sub-panel of the left panel, expression scatter-plot between MDAMB231 and  the "median" Basal-like samples
####################################################################
...
############################################################ 
#### Fig 4b: right  panel, boxplot of CNV correlation with Basal-like MET500 breast cancer samples, MDAMB231 and HCC70
###########################################################
...
############################################################ 
#### Fig 4b: left  panel, ranking cell lines according to median CNV correlation with Basal-like MET500 breast cancer samples
###########################################################
...
df$size       <- 1
df[rownames(df) == 'MDAMB231','size'] <- 4
ggplot(df) + geom_point(aes(x=rank,y=median.correlation,color=factor(color),size=size),show.legend=F) + ggplot.style + ylab('CNV similarity')+ xlab('rank') + scale_color_manual(values=c('red'='red','black'='black')) + scale_size_area(max_size = 4)
...
############################################################ 
#### Fig 4c: visulize mutation profiles of the 25 highly mutated genes in Basal-like MET500 samples
###########################################################
...
############################################################ 
#### Fig 4d Boxplot of expression correlation between cell lines (including lung-derived MDAMB231) and lung-metastais derived samples  #############
###########################################################
...
##############################################
#### FigS7 show the qqplot to demnostrated that we could use normal distribution to compute p-value
##############################################
...
#############################################
### Fig S8a  BT483 shows significantly higer correlation  with MET500 LumB samples than MCF7
#############################################
...
ggplot(df,aes(x=cell.line,y=cor)) + geom_boxplot(lwd=1.2) + geom_point(size=4, position=position_jitterdodge(jitter.width = 0.2),aes(fill=cell.line)) + ggplot.style + xlab('') + ylab('correlation') + ylim(c(0,0.6))
...
###################################################
# Fig S8b  MDAMB415 does NOT show significantly higer correlation with MET500 LumA samples than T47D
###################################################
cor.vec <- c(
...
###################################################
#### Fig S8c  EFM192A  show significantly higer correlation with MET500 Her2 samples than T47D
###################################################
